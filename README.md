...
- **探索地(`scenes/Wilds.tscn` + `scripts/AreaDB.gd`)**: 町の外れの分かれ道で行き先を選びます。3カ所とも同じシーンを使い、`AreaDB` の定義で見た目・採れる素材・けものの強さが変わります
  - **囁きの森**: 危険が少ない。風と水の薬草、木や石
  - **響きの岩窟**: けものが手強い。土と闇の鉱石、暗がりの薬草
  - **陽昇る霊峰**: 最も危険。光と火の希少な素材、そして核
...
- **アイテム108種**: 薬草・鉱石・霊薬・魔道具・触媒・素材・道具・食べ物。すべてに**属性**(火・水・風・土・光・闇・無)と**力**(1〜5)があります
- **錬成(魔法研究)**: 工房(C)の「錬成」タブは**図鑑と釜の二画面**です(`scripts/autoload/AlchemyDB.gd`)
  - 覚えたレシピを選ぶのではなく、**素材の性質そのもの**から結果が決まります
  - 釜に最大4つ入れると、属性ごとに力が合計され、最も強い属性が**主属性**になります
  - 主属性の合計が3以上で霊薬ができ、7以上・11以上と**階梯**が上がります
  - **純度**(主属性が全体に占める割合)が高いほど出来がよく、2個できます。混ぜすぎると澱んで失敗します
  - **触媒**は属性を持たない代わりに純度を底上げします。最上段かつ高純度で触媒を入れると**魔道具**(杖・護符)が生まれます
  - 左の図鑑で属性と力を見比べ、「あと火をいくつ足せば次の段か」を考えながら組み立てるのが遊びどころです
